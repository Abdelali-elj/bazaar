...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement> | FormData) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Use window.location to force a full hard page reload to ensure auth state syncs everywhere
  ...
  return (
    <main className="min-h-screen relative flex flex-col items-center pt-32 pb-12 bg-black overflow-y-auto px-4 md:px-0">
      ...
      {/* Floating Login Card - Smaller & Below Navbar */}
      <div className="w-full max-w-[1140px] relative z-10 flex flex-col md:flex-row bg-white rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden animate-fade-up my-auto">
        ...
        <div className="w-full md:w-[50%] relative min-h-[250px] md:min-h-[520px]">
          <Image
            ...
            alt="Sign In"
            fill
            className="object-cover transition-transform duration-[3000ms] scale-100 hover:scale-105"
        ...
        <div className="w-full md:w-[50%] bg-white p-8 md:p-10 lg:p-14 flex flex-col justify-center">
          ...
          {error && (
            <div className="bg-rose-500/5 border border-rose-500/10 text-rose-600 text-[10px] font-bold uppercase tracking-widest px-6 py-2.5 rounded-2xl mb-4 flex items-center gap-3 animate-shake">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 md:space-y-5">
            ...
            <div className="space-y-1.5 relative group">
              <div className="flex justify-between items-center ml-2">
                ...
                <Link href="#" className="text-[7px] md:text-[8px] font-bold text-black/20 hover:text-black uppercase tracking-widest transition-colors">Oublié ?</Link>
              </div>
              <div className="relative">
                <input 
                  name="password"
          ...
          <div className="mt-6 flex items-center justify-between border-t border-black/5 pt-6">
            <Link href="/" className="text-[8px] md:text-[9px] font-bold text-black/10 hover:text-black uppercase tracking-[0.4em] transition-all font-sans">
               Retour à l'accueil
            </Link>
            <div className="text-[9px] md:text-[10px] font-bold text-black/30 uppercase tracking-[0.2em]">
              Nouveau membre ?{" "}
              <Link href="/auth/register" className="text-[#AF8E5C] hover:text-[#967a4e] transition-all ml-1 border-b-2 border-[#AF8E5C]/20 pb-0.5">
                Rejoindre
              </Link>
            </div>
          </div>
