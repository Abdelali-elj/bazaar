...
async function scrapeProduct(url: string) {
    try {
        ...
        if (html.includes('Mobile Only Access') && !html.includes(':product=')) {
            console.log(`   [!] Blocked by bot protection for ${url}`);
            return null;
        }

        // Extract data from :product attribute
        ...
        if (!title || title.toLowerCase().includes('mobile only access')) return null;
...
async function migrate() {
    ...
    const sitemapCmd = `curl.exe -s https://bazaarstyle.ma/sitemap/products_01.xml`;
    const sitemapXml = execSync(sitemapCmd, { encoding: 'utf-8' });
    const productUrls = [...sitemapXml.matchAll(/<loc>(https:\/\/bazaarstyle\.ma\/products\/[^<]+)<\/loc>/g)].map(m => m[1]);
    
    console.log(`Found ${productUrls.length} products in sitemap.`);

    let successCount = 0;
    for (let i = 0; i < productUrls.length; i++) {
        const url = productUrls[i];
        process.stdout.write(`[${i+1}/${productUrls.length}] ${url}... `);
        ...
        try {
            ...
            if (product) {
                ...
                else if (lowTitle.includes('parfum') || lowTitle.includes('tester')) category_id = 'parfums';

                await db.collection('products').add({ ...product, category_id });
                console.log(`OK (${product.price} MAD)`);
                successCount++;
            } else {
                console.log(`SKIPPED`);
            }
        ...
        await sleep(500); // Reduced delay to 500ms since we have many products
    }

    console.log(`--- MIGRATION FINISHED: ${successCount} products imported ---`);
    process.exit(0);
}
